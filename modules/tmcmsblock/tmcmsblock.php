...
include_once _PS_MODULE_DIR_.'tmcmsblock/tminfoClass.php';

class Tmcmsblock extends Module
{
	public function __construct()
	{
		$this->name = 'tmcmsblock';
		...
		parent::__construct();
		$this->displayName = $this->l('TM - Header Nav CMS Block');
		$this->description = $this->l('Adds CMS Content anywhere');
		$this->ps_versions_compliancy = array('min' => '1.6', 'max' => _PS_VERSION_);
	}
	public function install()
	{
		return parent::install() && $this->installDB() && Configuration::updateValue('BLOCKCMSINFO_NBBLOCKS', 1) && $this->registerHook('displayNav') && $this->registerHook('displayHeader') && $this->installFixtures() && $this->disableDevice(Context::DEVICE_TABLET | Context::DEVICE_MOBILE);
	}
	public function installDB()
	{
		$return = true;
		$return &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tminfo` (
		...
		$return &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tminfo_lang` (
				`id_info` INT UNSIGNED NOT NULL AUTO_INCREMENT,
	...
	{
		return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tminfo`') && 
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tminfo_lang`');
	}
		...
		if (Tools::getValue('nbblocks') != true)
		{
			for ($i = 1; $i <= (int)Tools::getValue('nbblocks'); $i++)
				Db::getInstance()->execute('
					INSERT INTO `'._DB_PREFIX_.'tminfo` (`text`)
	...
	{
		return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'tminfo`');
	}
	public function getContent()
	{
		...
		if (Tools::isSubmit('savetmcmsblock'))
		{
			if ($id_info = Tools::getValue('id_info'))
				$info = new tminfoClass((int)$id_info);
			else
				$info = new tminfoClass();
			$info->copyFromPost();
			...
			{
				$info->save();
				$this->_clearCache('tmcmsblock.tpl');
			}
			else
				$html .= '<div class="conf error">'.$this->l('An error occurred while attempting to save.').'</div>';
		}
		if (Tools::isSubmit('updatetmcmsblock') || Tools::isSubmit('addtmcmsblock'))
		{
		...
		else if (Tools::isSubmit('deletetmcmsblock'))
		{
			$info = new tminfoClass((int)$id_info);
			$info->delete();
			$this->_clearCache('tmcmsblock.tpl');
	...
	protected function getListContent($id_lang, $id_shop = null)
	{
		$content = Db::getInstance()->executeS('
			...
			FROM `'._DB_PREFIX_.'tminfo` r
			LEFT JOIN `'._DB_PREFIX_.'tminfo_lang` rl ON (r.`id_info` = rl.`id_info`)
			WHERE `id_lang` = '.(int)$id_lang.($id_shop ? ' AND id_shop='.bqSQL((int)$id_shop) : '').
			(Tools::getIsset('tmcmsblockOrderby') && Tools::getIsset('tmcmsblockOrderway') ?
				' ORDER BY `'.bqSQL(Tools::getValue('tmcmsblockOrderby')).'` '.bqSQL(Tools::getValue('tmcmsblockOrderway')) : '')
		);
		return $content;
	}
	protected function initForm()
	{
		...
		$helper->module = $this;
		$helper->name_controller = 'tmcmsblock';
		...
		$helper->submit_action = 'savetmcmsblock';
	...
	{
		if (!$this->isCached('tmcmsblock.tpl', $this->getCacheId()))
		{
		...
		return $this->display(__FILE__, 'tmcmsblock.tpl', $this->getCacheId());
	}


	public function hookHeader($params)
	{
		$this->context->controller->addCSS($this->_path.'css/tmstyle.css', 'all');
	}
	public function installFixtures()
	{
		...
		$tab_texts = array(
			array(
				'text' => '<div class="nav_text"><a href="#">new offers this weekend only</a></div>'
			),
		);
