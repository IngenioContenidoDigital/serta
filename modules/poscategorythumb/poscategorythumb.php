<?php
class poscategorythumb extends Module {
	private $spacer_size = '5';	
	private $_postErrors  = array();
	public function __construct() {
		$this->name 		= 'poscategorythumb';
		$this->tab 			= 'front_office_features';
		$this->version 		= '1.6';
        $this->bootstrap    = true;
        $this->_html        = '';
		$this->author 		= 'posthemes';
		$this->displayName 	= $this->l('Category Tabs Slider With Image Holder');
             ...
             if($_FILES['imagethumb']['tmp_name']!=''){
                 ...
                 if(move_uploaded_file($_FILES['imagethumb']['tmp_name'],$upload_path .$filename)){
                     ...
                     if($cate_exit ==null){
                        ...
                        $this->_html .= $this->displayConfirmation($this->l('Add images success'));
                     }
                 }
             }
         }
    ...
    public  function addcategoryicon($id_cate,$name_cate,$filename,$id_shop){
        return   $res = Db::getInstance()->execute('INSERT  INTO `'._DB_PREFIX_.'pos_categorythumb`(`id_category`,`name_category`,`image`,`id_shop`)
                        VALUES ('.(int)$id_cate.', \''.pSQL($name_cate).'\', \''.pSQL($filename).'\','.(int)$id_shop.')');
    }
    ...
    public  function imageForm(){
        ...
        $fields_form = array(
            'form' => array(
                'legend' => array(
                    'title' => $this->l('Upload Image Thumb Categories'),
                    'icon' => 'icon-link'
                ),
                'input' => array(
                    array(
	...
	public  function _displayForm(){
        ...
        $fields_form = array(
            'form' => array(
                ...
                'legend' => array(
                    'title' => $this->l('Tab categories slider'),
                    'icon' => 'icon-link'
                ),
                'submit' => array(
                    'title' => $this->l('Save'),
    ...
    public function  getcategoryicon(){
        $sql = 'SELECT id_categoryslider ,image,id_category,name_category, id_shop
    ...
    public function  getimage($id,$id_shop){
        $sql = 'SELECT id_categoryslider ,image,id_category,name_category
				FROM '._DB_PREFIX_.'pos_categorythumb WHERE id_category = '.$id.' and id_shop ='.$id_shop.'' ;
        return Db::getInstance()->executeS($sql);
    }
