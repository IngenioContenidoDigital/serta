...
class Posmegamenu extends Module {
    ...
    public function install() {
        ...
        Configuration::updateValue($this->name . '_top_offset', 47);
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_hitstore/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						$this->_menuLink .= '<div class ="pt_menu_link pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</span></a></div></div>'.PHP_EOL;
					}
					break;
