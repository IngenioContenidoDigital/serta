...
include_once('tmsubbanner.php');

$context = Context::getContext();
$sub_banner = new TmSubBanner();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'tmsubbanner_slides` SET `position` = '.(int)$position.'
			WHERE `id_tmsubbanner_slides` = '.(int)$id_slide
		);
