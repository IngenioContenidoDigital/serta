...
require (dirname(__FILE__).'/tmleftmenu.class.php');



class tmLeftmenu extends Module

{
...

	{
		...
		$this->displayName = $this->l('TM - Left Menu');
...

	{
		...
		$success = (parent::install()
			...
			&& Configuration::updateGlobalValue('MOD_TESTBLOCKTOPMENU_ITEMS', 'CAT3,CAT26')
...

	{
		...
		if (!parent::uninstall() ||
			...
			!Configuration::deleteByName('MOD_TESTBLOCKTOPMENU_ITEMS') ||
...

	{
...

		{
			...
			if (is_array($items) && count($items))
 				...
 				$updated = Configuration::updateValue('MOD_TESTBLOCKTOPMENU_ITEMS', (string)implode(',', $items));

 			else

 				$updated = Configuration::updateValue('MOD_TESTBLOCKTOPMENU_ITEMS', '');
...

		{
...

			{
...

				{
					...
					tmMenuTopLinks::add($links_label, $labels,  Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
...

		{
			...
			tmMenuTopLinks::remove($id_linksmenutop, (int)Shop::getContextShopID());

			Configuration::updateValue('MOD_TESTBLOCKTOPMENU_ITEMS', str_replace(array('LNK'.$id_linksmenutop.',', 'LNK'.$id_linksmenutop), '', Configuration::get('MOD_TESTBLOCKTOPMENU_ITEMS')));
...

		{
...

			{
				...
				tmMenuTopLinks::update($link, $label, $new_window, (int)$id_shop, (int)$id_linksmenutop, (int)$id_linksmenutop);
		...
		$links = tmMenuTopLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
...

	{
...

		{
			...
			$conf = Configuration::get('MOD_TESTBLOCKTOPMENU_ITEMS');

			if (strlen($conf))

				return explode(',', Configuration::get('MOD_TESTBLOCKTOPMENU_ITEMS'));
	...
	private function makeMenutestOption()
...

		{
...

			{
				...
				case 'LNK':
					...
					$link = tmMenuTopLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
...

					{
...

						{
							...
							$link = tmMenuTopLinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
...

		{
...

			{
				...
				case 'CAT':
					...
					$this->_menutest .= $this->generateCategoriesMenu(Category::getNestedCategories($id, $id_lang, true, $this->user_groups));
				...
				case 'PRD':
					...
					if (!is_null($product->id))
						...
						$this->_menutest .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($product->getLink()).'" title="'.$product->name.'">'.$product->name.'</a></li>'.PHP_EOL;
				...
				case 'CMS':
					...
					if (count($cms))
						...
						$this->_menutest .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($cms[0]['link']).'" title="'.Tools::safeOutput($cms[0]['meta_title']).'">'.Tools::safeOutput($cms[0]['meta_title']).'</a></li>'.PHP_EOL;
				...
				case 'CMS_CAT':
...

					{
						...
						$this->_menutest .= '<li><a href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'" title="'.$category->name.'">'.$category->name.'</a>';

						$this->getCMSMenuItems($category->id);

						$this->_menutest .= '</li>'.PHP_EOL;
				...
				case 'ALLMAN':
					...
					$this->_menutest .= '<li><a href="'.$link->getPageLink('manufacturer').'" title="'.$this->l('All manufacturers').'">'.$this->l('All manufacturers').'</a><ul>'.PHP_EOL;

					$manufacturers = Manufacturer::getManufacturers();

					foreach ($manufacturers as $key => $manufacturer)

						$this->_menutest .= '<li><a href="'.$link->getManufacturerLink((int)$manufacturer['id_manufacturer'], $manufacturer['link_rewrite']).'" title="'.Tools::safeOutput($manufacturer['name']).'">'.Tools::safeOutput($manufacturer['name']).'</a></li>'.PHP_EOL;
				...
				case 'MAN':
...

					{
						...
						$this->_menutest .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'" title="'.Tools::safeOutput($manufacturer->name).'">'.Tools::safeOutput($manufacturer->name).'</a></li>'.PHP_EOL;
				...
				case 'ALLSUP':
					...
					$this->_menutest .= '<li><a href="'.$link->getPageLink('supplier').'" title="'.$this->l('All suppliers').'">'.$this->l('All suppliers').'</a><ul>'.PHP_EOL;

					$suppliers = Supplier::getSuppliers();

					foreach ($suppliers as $key => $supplier)

						$this->_menutest .= '<li><a href="'.$link->getSupplierLink((int)$supplier['id_supplier'], $supplier['link_rewrite']).'" title="'.Tools::safeOutput($supplier['name']).'">'.Tools::safeOutput($supplier['name']).'</a></li>'.PHP_EOL;
				...
				case 'SUP':
...

					{
						...
						$this->_menutest .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'" title="'.$supplier->name.'">'.$supplier->name.'</a></li>'.PHP_EOL;
				...
				case 'SHOP':
...

					{
						...
						$this->_menutest .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'" title="'.$shop->name.'">'.$shop->name.'</a></li>'.PHP_EOL;
				...
				case 'LNK':
...

					{
...

						{
							...
							$link = tmMenuTopLinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());

						}

						$this->_menutest .= '<li><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' onclick="return !window.open(this.href);"': '').' title="'.Tools::safeOutput($link[0]['label']).'">'.Tools::safeOutput($link[0]['label']).'</a></li>'.PHP_EOL;
	...
	private function generateCategoriesMenu($categories,$level = 1)

	{
...

		{
			...
			$selected = ($this->page_name == 'category' && (int)Tools::getValue('id_category') == (int)$category['id_category']) ? 'sfHoverForce' : '';
...

			{
				...
				$html .= $this->generateCategoriesMenu($category['children'],$level="1");	
...

	{
...

		{
...

			{
				...
				$this->_menutest .= '<a href="'.Tools::HtmlEntitiesUTF8($cat->getLink()).'">'.$category['name'].'</a>';
...

			{
				...
				$this->_menutest .= '<li '.$selected.'>';

				$this->_menutest .= '<a href="'.$links[0]['link'].'">'.$cms->meta_title.'</a>';
...

	{
		...
		return 'tmleftmenu|'.(int)Tools::usingSecureMode().'|'.$page_name.'|'.(int)$this->context->shop->id.'|'.implode(', ',$this->user_groups).'|'.(int)$this->context->language->id.'|'.(int)Tools::getValue('id_category').'|'.(int)Tools::getValue('id_manufacturer').'|'.(int)Tools::getValue('id_supplier').'|'.(int)Tools::getValue('id_cms').'|'.(int)Tools::getValue('id_product');
...

	{
		...
		$this->context->controller->addJS($this->_path.'js/tm_hoverIntent.js');

		$this->context->controller->addJS($this->_path.'js/tm_superfish-modified.js');

		$this->context->controller->addJS($this->_path.'js/tm_blocktopmenu.js');

		$this->context->controller->addCSS($this->_path.'css/tm_blocktopmenu.css');

		$this->context->controller->addCSS($this->_path.'css/tm_superfish-modified.css');
	...
	{
		//if ((!isset($this->context->controller->php_self) || $this->context->controller->php_self != 'index') && (!isset($this->context->controller->php_self) || $this->context->controller->php_self != 'category'))
		...
		if (!$this->isCached('tmleftmenu_left.tpl', $this->getCacheId()))

		{

			if (Tools::isEmpty($this->_menutest))

				$this->makeMenutest();

			$this->smarty->assign('TMMENU', $this->_menutest);

			$this->smarty->assign('this_path', $this->_path);

		}

		$html = $this->display(__FILE__, 'tmleftmenu_left.tpl', $this->getCacheId());
...

	{
		...
		if (!$this->isCached('tmleftmenu.tpl', $this->getCacheId()))
		...
		$html = $this->display(__FILE__, 'tmleftmenu.tpl', $this->getCacheId());
...

	{
		...
		$this->_clearCache('tmleftmenu.tpl');
...

	{
		...
		$helper->tpl_vars = array(
			...
			'selected_links' => $this->makeMenutestOption(),
...

	{
		...
		$links = tmMenuTopLinks::gets($this->context->language->id, null, (int)Shop::getContextShopID());
...

		{
...

			{
				...
				$link = tmMenuTopLinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
...

	{
...

		{
			...
			$link = tmMenuTopLinks::getLinkLang(Tools::getValue('id_linksmenutop'), (int)Shop::getContextShopID());
...

	{
		...
		$links = tmMenuTopLinks::gets((int)$this->context->language->id, null, (int)Shop::getContextShopID());
