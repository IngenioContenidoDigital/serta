...

{
...

	{
		...
		$this->displayName = $this->l('TM - Homepage Slider');

		$this->description = $this->l('Display Slider on Homepage.');
...

	{
...

		{
			...
			$res = Configuration::updateValue('HOMESLIDER_SPEED', '5000');
...

	{
...

		{
...

			{
				...
				$slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=v16_tmhomeslider';
...

	{
...

		{
			...
			/* Checks title/url/legend/for default lang */
...

	{
		...
		if (!$this->isCached('tmhomeslider.tpl', $this->getCacheId()))

		{
			...
			$this->smarty->assign(array('tmhomeslider_slides' => $slides));
...

	{
		...
		$this->context->controller->addCSS($this->_path.'css/flexslider.css');
		...
		$this->smarty->assign('tmhomeslider', $slider);
...

	{
		...
		return $this->display(__FILE__, 'tmhomeslider.tpl', $this->getCacheId());
...

	{
		...
		$this->_clearCache('tmhomeslider.tpl');
...

	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			SELECT hs.`id_tmhomeslider_slides` as id_slide, hssl.`image`, hss.`position`, hss.`active`, hssl.`title`,
