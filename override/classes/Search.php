...
{
	 /*
    * module: tmblocksearch
    * date: 2016-02-02 09:18:31
    * version: 1.5.3
    */
        ...
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
		...
		INNER JOIN `'._DB_PREFIX_.'category` c ON cp.`id_category` = c.`id_category`'.(Tools::getValue('search_category') ? ' AND c.`id_category` = '.(int)Tools::getValue('search_category') : '').'
        ...
        if ($ajax) {
            ...
            foreach($result_properties as &$res){
                $res['image'] = $context->link->getImageLink($res['link_rewrite'], $res['id_image'], 'cart_default');
